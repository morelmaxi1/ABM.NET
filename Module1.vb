...
Module Module1
    ...
    Function SQL_Accion(ByVal Sql_de_accion As String) As Boolean
        ...
        Dim conn As New SqlConnection("data source=" & CStr(leerarchivo(ar)) & "; initial catalog=Clientes; integrated security=true")
    ...
    Function Loguin(ByVal tipo As String, ByVal usuario As String, ByVal clave As String) As Boolean
        Dim ar As String
        Dim con As New SqlConnection("data source=" & CStr(leerarchivo(ar)) & "; initial catalog=Clientes; integrated security=true")
        usuario = usuario.Replace("'", "").Replace(" ", "").Replace("""", "")
        clave = clave.Replace("'", "").Replace(" ", "").Replace("""", "")

        Dim da As New SqlDataAdapter("SELECT * from " & tipo & " where [Usuario] ='" & usuario & "' and [clave] ='" & clave & "'", con)
